...
The implementation of the Quadratic Theil-Senn regression in `FullTSQuadraticSeries.js` contains an optimization that hurts its accuracy. In theory, all factors for a (in generalised formula y = a \* x<sup>2</sup> + b \* x + c ) should be considered in the median calculation. `TSQuadraticSeries` maintains a matrix of all possible `a`'s to determine the median. When calculating the optimal curve between points x<sub>1</sub> and x<sub>3</sub> by calculateA(x<sub>1</sub>, x<sub>3</sub>), all possible intermediate values of x<sub>2</sub> are considered, resulting in a series of `a`'s.
